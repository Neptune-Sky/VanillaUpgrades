...
namespace VanillaUpgrades
{
    ...
    {
        private const string posKey = "WorldClockWindow";
        private readonly int MainWindowID = Builder.GetRandomID();
        ...
        void Awake()
        {
            ...
            {
                clockWindow.rectTransform.localScale = new Vector2(Config.settingsData.persistentVars.windowScale, Config.settingsData.persistentVars.windowScale);
        ...
        void ShowGUI()
        {
            ...
            Vector2Int pos = Config.settingsData.windowsSavedPosition.GetValueOrDefault(posKey, new Vector2Int((int)WindowManager.gameSize.x, (int)WindowManager.gameSize.y - 70));
            clockWindow = Builder.CreateWindow(clockHolder.gameObject.transform, MainWindowID, 300, 220, pos.x, pos.y, true, true, 1, "World Clock");
            ...
            clockWindow.CreateLayoutGroup(SFS.UI.ModGUI.Type.Vertical, TextAnchor.UpperCenter, 10, new RectOffset(0, 0, 3, 0));
        ...
        void OnTimewarpButton()
        {
            ...
            if (timewarpIndex != 0)
            {
                if (!clockHolder.gameObject.activeSelf)
                {
                    clockHolder.gameObject.SetActive(true);
                }
            }
            else clockHolder.gameObject.SetActive(false);
        }

        void ToggleChecks()
        {
            if (!Config.settingsData.showTime || (WorldTime.main.timewarpIndex == 0 && !Config.settingsData.alwaysShowTime))
            {
                clockHolder.SetActive(false);
            }
            if (Config.settingsData.showTime && Config.settingsData.alwaysShowTime)
            {
        ...
        string defaultTime = "000d 00h 00m 00s";
